...
var Ubuntu2404AmdSevSnp = eventLog{
	RawLog: test.Ubuntu2404AmdSevSnptEventLog,
	Banks: []*pb.PCRs{{
		Hash: pb.HashAlgo_SHA1,
		Pcrs: map[uint32][]byte{
	...
	for _, log := range logs {
		for _, bank := range log.Banks {
			...
			t.Run(subtestName, func(t *testing.T) {
				...
				// check if the digest of EV_EVENT_TAG is in the GRUB files.
				// `LOADED_IMAGE::LoadOptions` is taken from https://github.com/torvalds/linux/blob/6a0e38264012809afa24113ee2162dc07f4ed22b/drivers/firmware/efi/libstub/efi-stub-helper.c#L227
				for _, f := range msState.Grub.GetFiles() {
					if strings.Contains(string(f.GetUntrustedFilename()), "LOADED_IMAGE::LoadOptions") && len(f.GetDigest()) != 0 {
						t.Error("the digest of EV_EVENT_TAG should not be in the GRUB files")
					}
				}
			})
		}
