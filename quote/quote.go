...
func Verify(q *pb.Quote, trustedPub crypto.PublicKey, extraData []byte) error {
...
func validatePCRDigest(quoteInfo *tpm2.QuoteInfo, pcrs *pb.PCRs, hash crypto.Hash) error {
	if !internal.SamePCRSelection(pcrs, quoteInfo.PCRSelection) {
		return fmt.Errorf("given PCRs and Quote do not have the same PCR selection")
	}
	pcrDigest := internal.PCRDigest(pcrs, hash)
