...
func (di *DriverInstaller) InstallGPUDrivers(ctx context.Context) error {
	...
	container, err := di.cdClient.NewContainer(
		...
		containerd.WithNewSpec(oci.WithImageConfig(image),
			...
			oci.WithProcessArgs("/cos-gpu-installer", "install", fmt.Sprintf("-version=%s", di.launchSpec.GpuDriverVersion), fmt.Sprintf("-host-dir=%s", InstallationHostDir), "--skip-nvidia-smi"),
