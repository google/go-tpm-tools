...
if [ -f /proc/driver/nvidia/version ]; then
    echo "nvidia version got" | tee /dev/console
    cat /proc/driver/nvidia/version | tee /dev/console
fi

export LD_LIBRARY_PATH=/opt/nvidia-host/590.48.01/lib64:$LD_LIBRARY_PATH


echo "Starting guest GPU services 590.48.01.." | tee /dev/console
...
ls -lathr /opt | tee /dev/console
ls -lathr /opt/nvidia | tee /dev/console
ls -lathr /opt/nvidia/nvlsm | tee /dev/console


ldd /opt/nvidia/nvlsm/sbin/nvlsm | tee /dev/console
