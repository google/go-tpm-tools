	...
	testCases := []struct {
		...
		},
		{
			...
			provider: &fakeGpuQuoteProvider{
				quote: &nvattestpb.GpuAttestationQuote{
					GpuInfos: []*nvattestpb.GpuInfo{
						validDevInfo("gpu-rtx-0", "vbios-rtx", "550.54.14", nvattestpb.GpuArchitectureType_GPU_ARCHITECTURE_HOPPER),
					},
				},
			},
			wantPass: true,
		...
		{
			name:       "failed due to nil quote provider",
			nonce:      []byte("nonce"),
			gpuType:    deviceinfo.H100,
			provider:   nil,
			wantPass:   false,
			wantErrStr: "nil GPU quote provider",
		},
		{
		...
		{
			name:    "failed due to nil devInfo",
			nonce:   []byte("nonce"),
			gpuType: deviceinfo.H100,
			provider: &fakeGpuQuoteProvider{
				quote: &nvattestpb.GpuAttestationQuote{
					GpuInfos: []*nvattestpb.GpuInfo{nil},
				},
			},
			wantPass:   false,
			wantErrStr: "nil GPU device info at index 0",
		},
		{
