...
func TestPayload(t *testing.T) {
	testCases := []struct {
		name                string
		blob                []byte
		originalImageDigest digest.Digest
	...
	}{
		{
			...
			originalImageDigest: "sha256:9494e567c7c44e8b9f8808c1658a47c9b7979ef3cceef10f48754fc2706802ba",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			sig := &Sig{
				...
				Blob:              tc.blob,
				OriginalImageDgst: tc.originalImageDigest,
			}
...
func TestWorkflow(t *testing.T) {
	...
	sig := &Sig{
		...
		Blob:              blob,
		OriginalImageDgst: digest.Digest(wantPayload.Critical.Image.DockerManifestDigest),
	}
