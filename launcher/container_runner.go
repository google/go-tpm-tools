...
func fetchContainerImageSignatures(ctx context.Context, sdClient *signaturediscovery.Client, targetRepos []string, logger *log.Logger) []internal.Signature {
	...
	// We need to deduplicate container image signatures.
	sigsMap := make(map[string]internal.Signature)
	for _, sigs := range signatures {
		for _, sig := range sigs {
			b64Sig, err := sig.Base64Encoded()
			if err != nil {
				logger.Printf("Failed to extract base64 encoded signature from oci.Signature %v: %v", sig, err)
			} else {
				sigsMap[b64Sig] = sig
			}
		}
	...
	for _, v := range sigsMap {
		foundSigs = append(foundSigs, v)
	}
	return foundSigs
}
