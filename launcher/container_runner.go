...
func (r *ContainerRunner) Run(ctx context.Context) error {
	...
	case spec.CloudLogging:
		...
		r.logger.Info("Container stdout/stderr redirected directly to Cloud Logging.")
	case spec.Serial:
	...
	if stdoutTarget != nil && stderrTarget != nil {
		var stdoutWriter, stderrWriter io.Writer = stdoutTarget, stderrTarget
		var asyncStdout, asyncStderr *logging.AsyncWriter
		if r.launchSpec.LogRedirect == spec.Everywhere || r.launchSpec.LogRedirect == spec.Serial {
			asyncStdout = logging.NewAsyncWriter(stdoutTarget, 8192)
			asyncStderr = logging.NewAsyncWriter(stderrTarget, 8192)
			stdoutWriter = asyncStdout
			stderrWriter = asyncStderr
		}
		defer func() {
			closeCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
			defer cancel()
			if asyncStdout != nil {
				_ = asyncStdout.Close(closeCtx)
			}
			if asyncStderr != nil {
				_ = asyncStderr.Close(closeCtx)
			}
