...
ctr run --rm --net-host --mount "type=bind,src=/tmp/container_launcher/,dst=/run/container_launcher/,options=rbind:rw" --env SERVICE_ROLE="SERVICE_ROLE_WSD" --env KEY_PROTECTION_MECHANISM="KEY_PROTECTION_VM" "$IMAGE_REF" "$CONTAINER_NAME" --kps-vm-ip="${KPS_VM_IP}" --socket="/run/container_launcher/kmaserver.sock"