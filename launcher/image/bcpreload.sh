...
append_cmdline() {
  ...
  mount "${efi_partition}" /mnt/disks/efi
...
main() {
  ...
  echo "[BC_PRELOAD] configured entrypoint"
  # Copy experiment file.
  copy_experiment_file
  echo "[BC_PRELOAD] copied experiment file"
  # Install container launcher.
  copy_launcher
  setup_launcher_systemd_unit
  echo "[BC_PRELOAD] copied launcher + systemd assets"
