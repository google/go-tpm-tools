...
func TestTPMDAOps(t *testing.T) {
	...
	err = SetTPMDAParams(rwc, 123 /*maxTries*/, 456 /*lockoutInterval*/, 789 /*lockoutRecovery*/)
	if err != nil {
		t.Fatal(err)
	}
	...
	expectedDaInfo = TPMDAParams{0 /*LockoutCounter*/, 123 /*maxTries*/, 456 /*lockoutInterval*/, 789 /*locakoutRecovery*/, true}
	if !cmp.Equal(*daInfo, expectedDaInfo) {
		t.Errorf("expected default DA parameters, got %+v, want %+v", daInfo, expectedDaInfo)
	}
}
