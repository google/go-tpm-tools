...
type Service struct {
	generateKEMKeypairFn func(bindingPubKey []byte) (uuid.UUID, []byte, error)
}

// NewService creates a new KPS KOL service with the given KCC function.
func NewService(generateKEMKeypairFn func(bindingPubKey []byte) (uuid.UUID, []byte, error)) *Service {
...
func (s *Service) GenerateKEMKeypair(bindingPubKey []byte) (uuid.UUID, []byte, error) {
	return s.generateKEMKeypairFn(bindingPubKey)
}
