...
type Service struct {
	generateKEMKeypairFn func(bindingPubKey []byte) (uuid.UUID, error)
}

// NewService creates a new KPS KOL service with the given KCC function.
func NewService(generateKEMKeypairFn func(bindingPubKey []byte) (uuid.UUID, error)) *Service {
...
func (s *Service) GenerateKEMKeypair(bindingPubKey []byte) (uuid.UUID, error) {
