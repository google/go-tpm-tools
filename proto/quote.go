...
func (x *Quote) Verify(trustedPub crypto.PublicKey, extraData []byte) error {
	sig, err := tpm2.DecodeSignature(bytes.NewBuffer(x.GetRawSig()))
	if err != nil {
	...
	case *ecdsa.PublicKey:
		...
		if err = verifyECDSAQuoteSignature(pub, hash, x.Quote, sig); err != nil {
			return err
		}
	case *rsa.PublicKey:
		...
		if err = verifyRSASSAQuoteSignature(pub, hash, x.Quote, sig); err != nil {
			return err
		}
	default:
	...
	attestationData, err := tpm2.DecodeAttestationData(x.GetQuote())
	if err != nil {
