...
func TestSignPSS(t *testing.T) {
	...
	keys := []struct {
		...
		{"RSA-SHA1", crypto.SHA1, templatePSS(tpm2.AlgSHA1), 1024},
		{"RSA-SHA256", crypto.SHA256, templatePSS(tpm2.AlgSHA256), 1024},
		{"RSA-SHA384", crypto.SHA384, templatePSS(tpm2.AlgSHA384), 1024},
		{"RSA-SHA512", crypto.SHA512, templatePSS(tpm2.AlgSHA512), 1024},
		{"RSA-SHA512-2048", crypto.SHA512, templatePSS(tpm2.AlgSHA512), 2048},
	}
	...
	for _, k := range keys {
		t.Run(k.name, func(t *testing.T) {
			...
			optsList := []crypto.SignerOpts{
				k.hash,
				&rsa.PSSOptions{SaltLength: rsa.PSSSaltLengthAuto, Hash: k.hash},
			}
			for _, opts := range optsList {
				...
				err = rsa.VerifyPSS(signer.Public().(*rsa.PublicKey), k.hash, digest, sig, nil)
				if err != nil {
					t.Error(err)
				}
			}
